...
let main (args) =
    ...
    | [ "Interpreter"; input ] ->
        ...
        0
    | [ "RegexToDfa"; input ] ->
        let input = JsonSerializer.Deserialize<Io.ce_regex_to_dfa.Input> input
        let output: Io.ce_regex_to_dfa.Output = RegexToDfa.analysis input
        Console.WriteLine("{0}", JsonSerializer.Serialize output)
        0
    | [ "RegexToEnfa"; input ] ->
        let input = JsonSerializer.Deserialize<Io.ce_regex_to_enfa.Input> input
        let output: Io.ce_regex_to_enfa.Output = RegexToEnfa.analysis input
        Console.WriteLine("{0}", JsonSerializer.Serialize output)
        0
    | [ "RegexToNfa"; input ] ->
        let input = JsonSerializer.Deserialize<Io.ce_regex_to_nfa.Input> input
        let output: Io.ce_regex_to_nfa.Output = RegexToNfa.analysis input
        Console.WriteLine("{0}", JsonSerializer.Serialize output)
        0
    | [ "RegexToDfaDirect"; input ] ->
        let input = JsonSerializer.Deserialize<Io.ce_regex_to_dfa_direct.Input> input
